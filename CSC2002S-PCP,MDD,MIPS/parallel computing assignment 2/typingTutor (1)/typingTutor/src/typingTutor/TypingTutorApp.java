...
//shared class variables
	...
	public static void setupGUI(int frameX,int frameY,int yLimit) {
		...
		gameWindow = new GamePanel(Hwords, words,xLimit, yLimit,done,started,won);
	   ...
	   textEntry.addActionListener(new ActionListener() {
	      public void actionPerformed(ActionEvent evt) { 
	    	  ...
	    	  if (!pause.get()) {
				  ...
				  CatchHungryWord catchHungryThread = new CatchHungryWord(text);
				  catchHungryThread.start();
	...
	public static void createWordMoverThreads() {
		...
		for (int i=0;i<noWords;i++) {
			...
			Hwords[i]=new HungryWord(dict.getNewWord(),xLimit,gameWindow.getValidYpos());
		}
		//create threads to move them
	    for (int i=0;i<noWords;i++) {
	    		wrdShft[i] = new WordMover(words[i],dict,score,startLatch,done,pause);
				hwrdShft[i] = new HungryWordMover(Hwords[i],dict,score,startLatch,done,pause);
	    }
        //word movers waiting on starting line
     	for (int i=0;i<noWords;i++) {
...
public static void main(String[] args) {
		...
		hwrdShft = new HungryWordMover[noWords];
		...
		CatchHungryWord.setWords(Hwords);  //class setter - static method
		CatchHungryWord.setScore(score);  //class setter - static method
		CatchHungryWord.setFlags(done,pause);
